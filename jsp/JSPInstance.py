...
class JSPInstance:
    ...
    def reset(cls, data, groups):
        ...
        cls.first_col = np.arange(start=0, stop=cls.number_of_tasks, step=1).reshape(cls.number_of_jobs, -1)[:, 0]
        cls.last_col = np.arange(start=0, stop=cls.number_of_tasks, step=1).reshape(cls.number_of_jobs, -1)[:, -1]
    ...
    def generate_rand_solution(cls):
        ...
        for i in range(cls.number_of_tasks):
            ...
            if action not in cls.last_col:
                candidate[action // cls.number_of_machines] += 1
            # 更新邻接表和机器-工序矩阵
            cls.update_by_action(action)

            if action % cls.number_of_machines == cls.number_of_machines - 1:  # 最后一道工序
                indices.remove(rand_idx)

        # 邻接表和opid都有了，获取各组的关键路径和分组最大的ms
        cls.cps, mss = gu.get_cps_mss_by_group(cls.dur, cls.g_list, cls.opIDsOnMchs, cls.groups)
    ...
    def update_by_action(cls, action):
        ...
        row = action // cls.number_of_machines
        col = action % cls.number_of_machines

        mch_a = cls.m[row][col] - 1  # 该工序操作的机器
